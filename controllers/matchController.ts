...
export async function updateMatch(req: Request, res: Response) {
  try {
    ...
    const match = await (Match as any).findById(id);
    if (!match) {
      return res.status(404).json({ error: "Match not found." });
    }

    if (homeTeamName) match.homeTeam.name = homeTeamName;
    if (homeTeamLogoUrl !== undefined) match.homeTeam.logo.url = homeTeamLogoUrl;
    if (awayTeamName) match.awayTeam.name = awayTeamName;
    if (awayTeamLogoUrl !== undefined) match.awayTeam.logo.url = awayTeamLogoUrl;
...
export async function updateMatchScore(req: Request, res: Response) {
  try {
    const { id } = req.params;
...
export async function deleteMatch(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const deleted = await (Match as any).findByIdAndDelete(id);
