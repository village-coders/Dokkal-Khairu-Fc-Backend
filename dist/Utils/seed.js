...
exports.seedDataIfEmpty = seedDataIfEmpty;
...
async function seedDataIfEmpty() {
    try {
        const newsCount = await News_1.News.countDocuments();
        if (newsCount === 0) {
          ...
          `,
            ...
            await News_1.News.insertMany(newsArticles);
            console.log("Successfully seeded news articles.");
        }
        const matchesCount = await Match_1.Match.countDocuments();
        if (matchesCount === 0) {
        ...
        await Player_1.Player.deleteMany({});
        await GalleryItem_1.GalleryItem.deleteMany({});
        ...
        await Player_1.Player.insertMany(players);
